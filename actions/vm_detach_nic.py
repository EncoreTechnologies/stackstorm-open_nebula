...
class VmDetachNic(BaseAction):
    def wait_for_nic(self, one, vm_id, detach_timeout):
        # Wait for nic to detach - LCM State of 3 is 'running'
        lcm_state = one.vm.info(vm_id).LCM_STATE
        start_time = time.time()
        while lcm_state != 3:
            # Set elapsed time for timeout check
            elapsed_time = time.time() - start_time
            if elapsed_time > detach_timeout:
                return lcm_state
    ...
    def run(self, detach_timeout, nic_id, vm_id, open_nebula=None):
        ...
        lcm_state = self.wait_for_nic(one, vm_id, detach_timeout)

...
class TemplateAttributeGet(BaseAction):
    def run(self, attribute_name, template_id, open_nebula=None):
        ...
        :returns: Template attribute value that corresponds with the given name
        """
        one = self.pyone_session_create(open_nebula)
        temp = one.template.info(template_id)

        if len(attribute_name) == 1:
            return_value = json.loads(json.dumps(getattr(temp, attribute_name[0])))
        else:
            return_value = temp
            # Check the template for the given attribute and raise an error if it isn't found

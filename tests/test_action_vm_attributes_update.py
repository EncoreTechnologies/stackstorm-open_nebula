...
from vm_attributes_update import VmAttributesUpdate
import unittest.mock as mock

__all__ = [
    'VmAttributesUpdateTestCase'
]


class VmAttributesUpdateTestCase(OneBaseActionTestCase):
    __test__ = True
    action_cls = VmAttributesUpdate
    ...
    def test_run(self, mock_session):
        ...
        result = action.run(attributes, vm_id, open_nebula)

        # Verify result and calls
        self.assertEqual(expected_result, result)
        mock_session.assert_called_with(open_nebula)
        mock_one.vm.update.assert_called_with(vm_id, attributes, 1)
